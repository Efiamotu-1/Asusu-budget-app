class ExpensesController < ApplicationController
  # before_action :set_expense, only: %i[show new create edit update destroy]

  before_action :authenticate_user!

  def index
    @expenses = Expense.all.order('expenses.created_at DESC')
  end

  def show
    @expense = Expense.find(params[:id])
  end

  def new
    @expense = Expense.new
  end

  def edit; end

  def create
    @expense = Expense.new(expense_params)
    @expense.author_id = current_user.id

    respond_to do |format|
      if @expense.save
        @category = CategoryExpense.create(expense_id: @expense.id,
                                           category_id: category_expense_params[:category_id])
        @category.increment_total
        format.html do
          redirect_to category_categories_expense_index_path(@category.category_id),
                      notice: 'Expense was successfully created.'
        end
      else
        format.html { render :new, status: :unprocessable_entity }
      end
    end
  end

  def update
    respond_to do |format|
      if @expense.update(expense_params)
        format.html { redirect_to category_path(@expense.category_id), notice: 'Expense was successfully updated.' }
      else
        format.html { render :edit, status: :unprocessable_entity }
      end
    end
  end

  def destroy
    @expense = Expense.find(params[:id])
    @expense.category_expenses.first.decrement_total
    @expense.destroy

    respond_to do |format|
      format.html do
        redirect_to category_categories_expense_index_path(@expense), notice: 'Expense was successfully destroyed.'
      end
    end
  end

  private

  def set_expense
    @expense = Expense.find(params[:id])
  end

  def expense_params
    params.require(:expense).permit(:name, :amount)
  end

  def category_expense_params
    params.require(:expense).permit(:category_id)
  end
end
